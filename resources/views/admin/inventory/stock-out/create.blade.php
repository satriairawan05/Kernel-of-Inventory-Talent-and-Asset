...
<div class="container-fluid py-4 page-shell">
    <section class="page-hero d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
        <div>
            ...
            <h2 class="mb-1">Tambah Barang Keluar</h2>
        </div>
        <span class="badge bg-white text-primary fs-6 px-3 py-2"><i class="fas fa-sign-out-alt me-1"></i> Stock Out</span>
    </section>

    <div class="card soft-card mt-4">
        <div class="card-header">
            <h4 class="mb-1">Form Barang Keluar</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('inventory.stock-out.store') }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror" placeholder="Catatan tambahan...">{{ old('notes') }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
