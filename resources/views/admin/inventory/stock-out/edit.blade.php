        ...
        <div class="card-body">
            <form action="{{ route('inventory.stock-out.update', $stockOut) }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="product_variant_id" name="product_variant_id" class="form-select @error('product_variant_id') is-invalid @enderror">
                            ...
                            @foreach ($productVariants as $variant)
                                <option value="{{ $variant->id }}" {{ old('product_variant_id', $stockOut->product_variant_id) == $variant->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        <label class="form-label" for="qty">Jumlah Keluar <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" id="qty" name="qty" value="{{ old('qty', abs($stockOut->qty)) }}" class="form-control @error('qty') is-invalid @enderror" placeholder="0.00">
                        <small class="text-muted">Stok saat ini: {{ number_format($stockOut->productVariant->stock->current_stock ?? 0, 2, ',', '.') }}</small>
                        @error('qty')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror" placeholder="Catatan tambahan...">{{ old('notes', $stockOut->notes) }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2 action-bar">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i> Update Barang Keluar</button>
                </div>
            </form>
        </div>
    </div>
