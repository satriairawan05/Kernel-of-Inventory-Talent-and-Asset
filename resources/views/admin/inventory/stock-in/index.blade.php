...
<div class="container-fluid py-4 page-shell">
    <section class="page-hero d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
        ...
        <div>
            <a href="{{ route('inventory.stock-in.create') }}" class="btn btn-success"><i class="fas fa-plus me-1"></i> Tambah Barang Masuk</a>
        </div>
    </section>
                        ...
                        @forelse($stockIns as $item)
                            <tr>
                                ...
                                <td class="action-buttons d-md-flex flex-md-row align-items-md-center gap-2">
                                    <a href="{{ route('inventory.stock-in.edit', $item) }}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i> Edit</a>
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $item->id }}"><i class="fas fa-trash"></i> Delete</button>
                                    <!-- Modal Delete -->
                                    <div class="modal fade" id="deleteModal{{ $item->id }}" tabindex="-1" data-bs-backdrop="static" aria-labelledby="deleteModalLabel{{ $item->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow">
                                                <div class="modal-header bg-danger">
                                                    <h5 class="modal-title text-white" id="deleteModalLabel{{ $item->id }}"><i class="fas fa-triangle-exclamation me-2"></i> Hapus Transaksi</h5>
                                                ...
                                                <div class="modal-body text-center py-4">
                                                    ...
                                                    <p class="text-muted mb-0">Anda akan menghapus transaksi barang masuk untuk <strong>{{ $item->productVariant->product->product_name ?? '-' }}</strong>.</p>
                                                    <p class="text-danger small mt-2 mb-0">Tindakan ini tidak dapat dibatalkan.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Batal</button>
                                                    <form action="{{ route('inventory.stock-in.destroy', $item) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
