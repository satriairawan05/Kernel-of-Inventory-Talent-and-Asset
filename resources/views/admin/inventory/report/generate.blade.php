            ...
            <form action="{{ route('inventory.report.generate') }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="location" class="form-control @error('location') is-invalid @enderror" value="{{ old('location') }}" placeholder="Cabang / Toko" required>
                        @error('location')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Report by <span class="text-danger">*</span></label>
                        <input type="text" name="reported_by" class="form-control @error('reported_by') is-invalid @enderror" value="{{ old('reported_by') }}" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="cashier_name" class="form-control @error('cashier_name') is-invalid @enderror" value="{{ old('cashier_name') }}" required>
                        @error('cashier_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Open at</label>
                        <input type="datetime-local" name="opened_at" class="form-control @error('opened_at') is-invalid @enderror" value="{{ old('opened_at') }}">
                        @error('opened_at')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Close at</label>
                        <input type="datetime-local" name="closed_at" class="form-control @error('closed_at') is-invalid @enderror" value="{{ old('closed_at') }}">
                ...
                <div class="mt-4 d-flex gap-2">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i> Generate Report</button>
                </div>
            </form>
        </div>
    </div>
