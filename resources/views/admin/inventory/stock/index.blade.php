...
@section('content')
    <div class="container-fluid py-4 page-shell">
        <section class="page-hero d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
            ...
            @if ($access['Create'] == 1)
                <a href="{{ route('inventory.stock.create') }}" class="btn btn-success"><i class="fas fa-plus me-1"></i> Add
                    New Stock</a>
            @endif
        </section>
                                        ...
                                        <td class="action-buttons d-md-flex flex-md-row align-items-md-center gap-2">
                                            ...
                                            @if ($access['Delete'] == 1)
                                                <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteStockModal{{ $stock->id }}"><i
                                                        class="fas fa-trash"></i> Delete</button>
                                                <!-- Modal Delete -->
                                                <div class="modal fade" id="deleteStockModal{{ $stock->id }}"
                                                    tabindex="-1" data-bs-backdrop="static"
                                                    aria-labelledby="deleteStockModalLabel{{ $stock->id }}"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content border-0 shadow">
                                                            <div class="modal-header bg-danger">
                                                                <h5 class="modal-title text-white"
                                                                    id="deleteStockModalLabel{{ $stock->id }}"><i
                                                            ...
                                                            <div class="modal-body text-center py-4">
                                                                ...
                                                                <p class="text-muted mb-0">You are about to delete stock for
                                                                    <strong>{{ $stock->productVariant->variant_name ?? 'variant' }}</strong>.
                                                                </p>
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <form
                                                                    action="{{ route('inventory.stock.destroy', $stock) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
