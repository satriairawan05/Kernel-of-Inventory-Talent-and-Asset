...
@push('js')
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const defaultPreview = document.getElementById('defaultPreview'); // Jika ada elemen default

            if (!input || !preview) return;

            // Simpan URL gambar default
...
@section('content')
    <div class="container-fluid py-4 page-shell">
        ...
        <div class="card soft-card mt-4">
            ...
            <div class="card-body">
                <form action="{{ route('inventory.product.update', $product) }}" method="POST"
                    enctype="multipart/form-data" class="row g-3">
                    @csrf
                    @method('PUT')
                    <div class="col-md-6">
                        <label class="form-label">Outlet</label>
                        <select name="company_id" class="form-select select2 @error('company_id') is-invalid @enderror"
                            required>
                    ...
                    <div class="col-12">
                        ...
                        <select name="has_variant" class="form-select select2 @error('has_variant') is-invalid @enderror"
                            required>
                            <option value="1" {{ old('has_variant', $product->has_variant) ? 'selected' : '' }}>Yes
                            </option>
                            <option value="0" {{ old('has_variant', $product->has_variant) ? '' : 'selected' }}>No
                            </option>
                        </select>
                        @error('has_variant')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="d-flex justify-content-center mt-3">
                            <img id="imagePreview" class="preview-box"
                                src="{{ $product->image ? asset('storage/' . $product->image) : 'https://placehold.co/150x150?text=Product+Image' }}"
