...
@section('content')
    <div class="container-fluid py-4 page-shell">
        <section class="page-hero d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
            ...
            @if ($access['Create'] == 1)
                <a href="{{ route('setting.unit.create') }}" class="btn btn-success"><i class="fas fa-plus me-1"></i> Add
                    New</a>
            @endif
        </section>
                                ...
                                @forelse($units as $unit)
                                    <tr>
                                        ...
                                        <td class="action-buttons d-md-flex flex-md-row align-items-md-center gap-2">
                                            @if ($access['Update'] == 1)
                                                <a href="{{ route('setting.unit.edit', $unit) }}"
                                                    class="btn btn-sm btn-primary"><i class="fas fa-edit"></i> Edit</a>
                                            @endif
                                            @if ($access['Delete'] == 1)
                                                <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteUnitModal{{ $unit->id }}"><i
                                                        class="fas fa-trash"></i> Delete</button>
                                                <div class="modal fade" id="deleteUnitModal{{ $unit->id }}"
                                                    tabindex="-1" data-bs-backdrop="static"
                                                    aria-labelledby="deleteUnitModalLabel{{ $unit->id }}"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content border-0 shadow">
                                                            <div class="modal-header bg-danger">
                                                                <h5 class="modal-title text-white"
                                                                    id="deleteUnitModalLabel{{ $unit->id }}"><i
                                                                        class="fas fa-triangle-exclamation me-2"></i> Delete
                                                                    Unit</h5><button class="btn p-1" type="button"
                                                            ...
                                                            <div class="modal-body text-center py-4">
                                                                ...
                                                                <p class="text-muted mb-0">You are about to delete
                                                                    <strong>{{ $unit->unit_name }}</strong>.</p>
