...
@section('content')
    <div class="container-fluid py-4 page-shell">
        <section class="page-hero d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
            ...
            @if ($access['Create'] == 1)
                <a href="{{ route('setting.role.create') }}" class="btn btn-success"><i class="fas fa-plus me-1"></i> Add New
                    Role</a>
            @endif
        </section>
                                        ...
                                        <td class="action-buttons d-md-flex flex-md-row align-items-md-center gap-2">
                                            ...
                                            @if ($access['Delete'] == 1)
                                                <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteRoleModal{{ $role->group_id }}">
                                                    <i class="fas fa-trash me-1"></i> Delete
                                                </button>

                                                <div class="modal fade" id="deleteRoleModal{{ $role->group_id }}"
                                                    tabindex="-1" data-bs-backdrop="static"
                                                    aria-labelledby="deleteRoleModalLabel{{ $role->group_id }}"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content border-0 shadow">
                                                            <div class="modal-header bg-danger">
                                                                <h5 class="modal-title text-white"
                                                                    id="deleteRoleModalLabel{{ $role->group_id }}">
                                                            ...
                                                            <div class="modal-body text-center py-4">
                                                                <div class="mb-3">
                                                                    <i class="fas fa-shield-alt text-danger"
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <form
                                                                    action="{{ route('setting.role.destroy', $role->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center py-4 text-muted">
                                            <i class="fas fa-folder-open d-block mb-2 text-secondary fs-3"></i> No Data
                                            Found
                                        </td>
                                    </tr>
                                @endforelse
