<form action="{{ $formAction }}" method="POST" onsubmit="btnsubmit.disabled=true; return true;">
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-md-2">
            <label class="form-label mb-md-0 fw-semibold" for="group_name">Role Name <span class="text-danger">*</span></label>
        </div>
        <div class="col-md-10">
                            ...
                            <tr>
                                <td class="fw-semibold text-dark ps-4">
                                    <div class="d-flex align-items-center gap-3">
                                        <input type="checkbox" class="form-check-input row-select-all shadow-sm" title="Select all permissions for this page">
                                    ...
                                    <div class="d-flex flex-wrap align-items-center gap-3 py-1">
                                        ...
                                        @foreach (['Create', 'Read', 'Update', 'Delete'] as $actionName)
                                            @foreach (collect($pages)->where('page_name', $d->page_name)->where('module', $moduleName)->where('action', $actionName) as $p)
                                                <div class="d-flex align-items-center gap-2 bg-light border border-light-subtle rounded px-3 py-1.5 shadow-sm">
                                                    <input type="checkbox" class="form-check-input m-0 permission-checkbox" id="{!! $p->page_id !!}" name="{!! $p->page_id !!}" {!! (isset($p->access) && $p->access == 1) ? 'checked' : '' !!}>
                                                    <span class="small fw-semibold text-dark text-capitalize ms-1">{{ $actionName }}</span>
                                                </div>
                                            @endforeach
                                        @endforeach
