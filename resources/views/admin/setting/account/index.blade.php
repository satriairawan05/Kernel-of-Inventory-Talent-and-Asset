...
<div class="container-fluid py-4 page-shell">
    <section class="page-hero d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
        ...
        <a href="{{ route('setting.account.create') }}" class="btn btn-success"><i class="fas fa-plus me-1"></i> Add New</a>
    </section>
    ...
    <div class="card soft-card mt-4">
        <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
            ...
            <span class="stat-chip bg-primary-subtle text-primary"><i class="fas fa-users me-1"></i> {{ $accounts->count() }} accounts</span>
        </div>
        <div class="card-body">
                        ...
                        @forelse($accounts as $account)
                            <tr>
                                ...
                                <td class="action-buttons d-md-flex flex-md-row align-items-md-center gap-2">
                                    <a href="{{ route('setting.account.edit', $account) }}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i> Edit</a>
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteAccountModal{{ $account->id }}"><i class="fas fa-trash"></i> Delete</button>

                                    <div class="modal fade" id="deleteAccountModal{{ $account->id }}" tabindex="-1" data-bs-backdrop="static" aria-labelledby="deleteAccountModalLabel{{ $account->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow">
                                                <div class="modal-header bg-danger">
                                                    <h5 class="modal-title text-white" id="deleteAccountModalLabel{{ $account->id }}"><i class="fas fa-triangle-exclamation me-2"></i> Delete Account</h5>
                                                ...
                                                <div class="modal-body text-center py-4">
                                                    ...
                                                    <p class="text-muted mb-0">You are about to delete <strong>{{ $account->name }}</strong>.</p>
                                                    <p class="text-danger small mt-2 mb-0">This action cannot be undone.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                                                    <form action="{{ route('setting.account.destroy', $account) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
