...
<div class="container-fluid py-4 page-shell">
    ...
    <div class="card soft-card mt-4">
        <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
            ...
            <span class="stat-chip bg-primary-subtle text-primary"><i class="fas fa-clock me-1"></i> {{ $shifts->count() }} shifts</span>
        </div>
        <div class="card-body">
                        ...
                        @forelse($shifts as $shift)
                            <tr>
                                ...
                                <td class="action-buttons d-md-flex flex-md-row align-items-md-center gap-2">
                                    <a href="{{ route('setting.shift.edit', $shift) }}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i> Edit</a>
                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteShiftModal{{ $shift->id }}"><i class="fas fa-trash"></i> Delete</button>
                                    <div class="modal fade" id="deleteShiftModal{{ $shift->id }}" tabindex="-1" data-bs-backdrop="static" aria-labelledby="deleteShiftModalLabel{{ $shift->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content border-0 shadow">
                                                <div class="modal-header bg-danger"><h5 class="modal-title text-white" id="deleteShiftModalLabel{{ $shift->id }}"><i class="fas fa-triangle-exclamation me-2"></i> Delete Shift</h5><button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close"><i class="fas fa-times text-white"></i></button></div>
                                                <div class="modal-body text-center py-4">
                                                    <div class="mb-3"><i class="fas fa-trash-alt text-danger" style="font-size: 4rem;"></i></div>
                                                    <h5 class="mb-3">Are you sure?</h5>
                                                    <p class="text-muted mb-0">You are about to delete <strong>{{ $shift->shift_name }}</strong>.</p>
                                                    <p class="text-danger small mt-2 mb-0">This action cannot be undone.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                                                    <form action="{{ route('setting.shift.destroy', $shift) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
