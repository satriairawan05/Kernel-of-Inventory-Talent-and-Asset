    ...
    <div class="page-shell">
        ...
        <div class="card hero-card mb-4">
            <div class="card-body">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-3">
                    <div>
                        <span class="chip mb-2"><i class="fas fa-calendar-alt"></i> Monthly Detail Report</span>
                        <h3 class="mb-1">Monthly Sales Overview</h3>
                        <p class="mb-0 text-white-50">Period: {{ $startDate }} to {{ $endDate }} @if($companyId) • Company ID {{ $companyId }} @endif</p>
                    </div>
                    <a href="{{ route('pos.report.monthly') }}" class="btn btn-light">
                        <i class="fas fa-arrow-left me-1"></i> Back to Monthly List
                    </a>
                </div>
                ...
                <div class="row g-3 mt-3">
                    <div class="col-md-3 col-sm-6">
                        <div class="kpi-card">
                            <small><i class="fas fa-calendar-alt me-1"></i> Total Months</small>
                            <h3>{{ $report['summary']['total_months'] ?? 0 }}</h3>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="kpi-card">
                    ...
                    <div class="col-md-3 col-sm-6">
                        <div class="kpi-card">
                            <small><i class="fas fa-chart-line me-1"></i> Average / Month</small>
                            <h3>{{ \Carbon\Carbon::rupiah(($report['summary']['total_months'] ?? 0) > 0 ? ($report['summary']['grand_total'] ?? 0) / ($report['summary']['total_months'] ?? 1) : 0, 0, ',', '.') }}</h3>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card report-card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0"><i class="fas fa-table me-2"></i> Monthly Period Breakdown</h5>
                    <small class="text-muted" style="color: #047857 !important;">Each row represents one month within the selected range.</small>
                </div>
                        ...
                        <tbody>
                            @forelse($report['data'] ?? [] as $index => $item)
                                <tr>
                                    ...
                                    <td>
                                        <strong>{{ $item['month_display'] ?? $item['month_name'] ?? '-' }}</strong>
                                        <br>
                                        <small class="text-muted">{{ $item['month_year'] ?? '-' }}</small>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5 text-muted">No monthly detail data found for the selected period.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
